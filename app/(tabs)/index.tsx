...
import { Ionicons } from "@expo/vector-icons"
import { useState, useRef, useEffect } from "react"
import { useRoute, type RouteProp, useNavigation, useFocusEffect } from "@react-navigation/native"
import type { NativeStackNavigationProp } from "@react-navigation/native-stack"
import AsyncStorage from "@react-native-async-storage/async-storage"
import { auth } from "../../firebaseConfig"
...
export default function HomeScreen() {
  ...
  // ID do usuário atual para armazenamento específico
  ...
  const saveUserData = async (data: UserData) => {
    try {
      console.log(`[HomeScreen] Salvando dados para usuário: ${currentUserId}`, data)

      // Sempre salvar no AsyncStorage para acesso offline
      await AsyncStorage.setItem(`userData_${currentUserId}`, JSON.stringify(data))
      ...
      if (currentUserId !== "guest" && auth.currentUser) {
        try {
          ...
          console.log(`[HomeScreen] Dados salvos no Firebase para ${currentUserId}`)
        } catch (firebaseError) {
          console.error(`[HomeScreen] Erro ao salvar no Firebase:`, firebaseError)
      ...
      console.error(`[HomeScreen] Erro ao salvar dados do usuário:`, error)
    }
  }
  ...
  const loadUserData = async () => {
    try {
      setIsLoading(true)
      console.log(`[HomeScreen] Carregando dados para usuário: ${currentUserId}`)
      ...
      if (currentUserId !== "guest") {
        try {
          ...
          if (userDoc.exists()) {
            ...
            console.log(`[HomeScreen] Dados carregados do Firebase:`, firebaseData)

            setUserData({
              name: firebaseData.name || "Usuário",
              weight: firebaseData.weight || 0,
              height: firebaseData.height || 0,
              waterIntake: firebaseData.waterIntake || 0,
            })

            // Calcular o consumo diário com base nos dados carregados
            if (firebaseData.waterIntake) {
              setRecommendedWaterIntake(firebaseData.waterIntake)
              setIsProfileConfigured(true)
              console.log(`[HomeScreen] Perfil configurado via Firebase: waterIntake = ${firebaseData.waterIntake}`)
            } else if (firebaseData.weight && firebaseData.height) {
              const intake = calculateWaterIntake(firebaseData.weight, firebaseData.height)
              setRecommendedWaterIntake(intake)
              setIsProfileConfigured(true)
              console.log(`[HomeScreen] Perfil configurado via Firebase: calculado = ${intake}`)
            } else {
              setIsProfileConfigured(false)
              console.log(`[HomeScreen] Perfil não configurado via Firebase`)
            }
          ...
          console.error(`[HomeScreen] Erro ao carregar dados do Firebase:`, error)
      ...
      const jsonValue = await AsyncStorage.getItem(`userData_${currentUserId}`)
      if (jsonValue !== null) {
        const data = JSON.parse(jsonValue) as UserData
        console.log(`[HomeScreen] Dados carregados do AsyncStorage:`, data)
        ...
        if (data.waterIntake) {
          setRecommendedWaterIntake(data.waterIntake)
          setIsProfileConfigured(true)
          console.log(`[HomeScreen] Perfil configurado via AsyncStorage: waterIntake = ${data.waterIntake}`)
        } else if (data.weight && data.height) {
          const intake = calculateWaterIntake(data.weight, data.height)
          setRecommendedWaterIntake(intake)
          setIsProfileConfigured(true)
          console.log(`[HomeScreen] Perfil configurado via AsyncStorage: calculado = ${intake}`)
        } else {
          setIsProfileConfigured(false)
          console.log(`[HomeScreen] Perfil não configurado via AsyncStorage`)
        }

        setIsLoading(false)
        return data
      } else {
        console.log(`[HomeScreen] Nenhum dado encontrado no AsyncStorage para ${currentUserId}`)
        setIsProfileConfigured(false)
      }
    } catch (error) {
      console.error(`[HomeScreen] Erro ao carregar dados do usuário:`, error)
      setIsProfileConfigured(false)
    }
    setIsLoading(false)
    return null
  ...
  const loadLastResetTimestamp = async () => {
    try {
      const timestamp = await AsyncStorage.getItem(`lastResetTimestamp_${currentUserId}`)
      if (timestamp) {
        setLastResetTimestamp(Number(timestamp))
      }
    } catch (error) {
      console.error(`[HomeScreen] Erro ao carregar timestamp do último reinício:`, error)
    }
  }

  // Função para lidar com a mudança de usuário
  const handleUserChange = async (userId: string) => {
    console.log(`[HomeScreen] Mudança de usuário detectada, resetando dados para o novo usuário: ${userId}`)

    // Atualizar o ID do usuário atual
    setCurrentUserId(userId)

    // Resetar os estados para o novo usuário
    setTotalWaterConsumed(0)
    setHistoryItems([])
    setGoalCelebrated(false)
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      setCurrentUserId(userId)
      console.log(`[HomeScreen] Inicializando dados para usuário: ${userId}`)
      ...
      try {
        if (user) {
          console.log(`[HomeScreen] Usuário autenticado:`, user.displayName)
          // Atualizar o nome do usuário no estado
          setUserData((prev) => ({
            ...prev,
            name: user.displayName || user.email?.split("@")[0] || "Usuário",
          ...
          console.log(`[HomeScreen] Nenhum usuário autenticado`)
        }
      } catch (error) {
      ...
      if (route.params?.waterIntake) {
        console.log(`[HomeScreen] Recebido waterIntake dos parâmetros da rota:`, route.params.waterIntake)
        setRecommendedWaterIntake(route.params.waterIntake)
        const updatedUserData = { ...userData }
        updatedUserData.waterIntake = route.params.waterIntake
        setUserData(updatedUserData)
        setIsProfileConfigured(true)
        console.log(`[HomeScreen] Perfil configurado via parâmetros da rota`)

        try {
          await saveUserData(updatedUserData)
        } catch (error) {
          console.error(`[HomeScreen] Erro ao salvar dados de consumo de água:`, error)
        }
      } else {
        try {
          const data = await loadUserData()

          if (!data || !isProfileConfigured) {
            console.log(`[HomeScreen] Perfil não configurado, mostrando alerta`)
            Alert.alert(
              "Perfil não configurado",
              "Por favor, configure seu perfil para calcular sua meta de consumo de água diário.",
              [
                {
          ...
          console.error(`[HomeScreen] Erro ao carregar dados do usuário:`, error)
          Alert.alert("Erro", "Erro ao carregar dados. Por favor, configure seu perfil.", [
            {
              text: "Configurar agora",
              onPress: () => goToProfile(),
            },
  ...
  // Adicione useFocusEffect para recarregar dados quando a tela recebe foco
  useFocusEffect(
    React.useCallback(() => {
      console.log("[HomeScreen] Tela recebeu foco, recarregando dados do usuário")
      loadUserData()
      loadHistoryItems()
    }, []),
  )
  ...
  useEffect(() => {
    ...
    const unsubscribe = auth.onAuthStateChanged((user) => {
      const userId = user ? user.uid : "guest"
      console.log(`[HomeScreen] Auth state changed - user: ${userId}`)

      if (user) {
        console.log(`[HomeScreen] Usuário logado:`, user.displayName)
        setUserData((prev) => ({
          ...prev,
          name: user.displayName || user.email?.split("@")[0] || "Usuário",
        }))

        // Resetar e carregar dados para o novo usuário
        handleUserChange(user.uid)
      } else {
        console.log(`[HomeScreen] Usuário deslogado`)

        // Resetar para usuário convidado
  ...
  const initializeAchievementsAndChallenges = async () => {
    try {
      const savedAchievements = await AsyncStorage.getItem(`achievements_${currentUserId}`)
      const savedChallenges = await AsyncStorage.getItem(`challenges_${currentUserId}`)

      if (!savedAchievements) {
        await AsyncStorage.setItem(`achievements_${currentUserId}`, JSON.stringify(initialAchievements))
        console.log(`[HomeScreen] Conquistas inicializadas com sucesso para ${currentUserId}`)
      }

      if (!savedChallenges) {
        await AsyncStorage.setItem(`challenges_${currentUserId}`, JSON.stringify(initialChallenges))
        console.log(`[HomeScreen] Desafios inicializados com sucesso para ${currentUserId}`)
      }
    } catch (error) {
      console.error(`[HomeScreen] Erro ao inicializar conquistas e desafios:`, error)
    }
  }
  ...
  const checkDailyReset = async () => {
    try {
      const lastResetDate = await AsyncStorage.getItem(`lastResetDate_${currentUserId}`)
      const today = new Date().toDateString()

      if (lastResetDate !== today) {
        console.log(`[HomeScreen] Novo dia detectado para ${currentUserId}, resetando dados diários`)

        // É um novo dia, resetar o contador e as conquistas
        await resetWaterConsumed(true) // true indica que é um reset diário

        // Salvar a data atual como último reset
        await AsyncStorage.setItem(`lastResetDate_${currentUserId}`, today)
      }
    } catch (error) {
      console.error(`[HomeScreen] Erro ao verificar reset diário:`, error)
    }
  }

  // Carregar itens do histórico do AsyncStorage e Firebase
  const loadHistoryItems = async () => {
    try {
      console.log(`[HomeScreen] Carregando histórico para: ${currentUserId}`)

      // Primeiro tentar carregar do Firebase se o usuário estiver logado
      if (currentUserId !== "guest") {
        try {
          const userDocRef = doc(firestore, "users", currentUserId)
          const userDoc = await getDoc(userDocRef)

          if (userDoc.exists() && userDoc.data().historyItems) {
            const items = userDoc.data().historyItems as HistoryItem[]
            console.log(`[HomeScreen] Histórico carregado do Firebase: ${items.length} itens`)
          ...
          console.error(`[HomeScreen] Erro ao carregar histórico do Firebase:`, error)
          // Continuar para tentar carregar do AsyncStorage
        }
      }

      // Se não conseguiu carregar do Firebase ou é usuário convidado, tentar do AsyncStorage
      const savedItems = await AsyncStorage.getItem(`historyItems_${currentUserId}`)
      if (savedItems) {
        const items = JSON.parse(savedItems) as HistoryItem[]
        console.log(`[HomeScreen] Histórico carregado do AsyncStorage: ${items.length} itens`)
        ...
        // Se não houver itens para este usuário, inicializar com array vazio
        console.log(`[HomeScreen] Nenhum histórico encontrado para ${currentUserId}`)
        setHistoryItems([])
        setTotalWaterConsumed(0)
      }
    } catch (error) {
      console.error(`[HomeScreen] Erro ao carregar histórico:`, error)
    }
  }
  ...
  const saveHistoryItems = async (items: HistoryItem[]) => {
    try {
      console.log(`[HomeScreen] Salvando histórico para: ${currentUserId} (${items.length} itens)`)

      // Sempre salvar no AsyncStorage para acesso offline
      await AsyncStorage.setItem(`historyItems_${currentUserId}`, JSON.stringify(items))
      ...
      if (currentUserId !== "guest" && auth.currentUser) {
        try {
          ...
          if (userDoc.exists()) {
            // Atualizar apenas o campo historyItems
            await updateDoc(userDocRef, { historyItems: items })
          } else {
            // Criar o documento com o campo historyItems
            await setDoc(userDocRef, { historyItems: items }, { merge: true })
          }

          console.log(`[HomeScreen] Histórico salvo no Firebase para ${currentUserId}`)
        } catch (error) {
          console.error(`[HomeScreen] Erro ao salvar histórico no Firebase:`, error)
        }
      }
      ...
      console.error(`[HomeScreen] Erro ao salvar histórico:`, error)
    }
  }
  ...
  const updateAchievements = async (items: HistoryItem[]) => {
    try {
      ...
      console.log(
        `[HomeScreen] Consumo total de hoje para ${currentUserId}: ${todayTotal}L, Meta: ${recommendedWaterIntakeLiters}L`,
      )

      // Verificar dias consecutivos
      let consecutiveDays = 0
      const lastConsecutiveDaysStr = await AsyncStorage.getItem(`consecutiveDays_${currentUserId}`)
      if (lastConsecutiveDaysStr) {
        consecutiveDays = Number.parseInt(lastConsecutiveDaysStr)
      }
      ...
      if (todayTotal >= recommendedWaterIntakeLiters) {
        consecutiveDays += 1
        await AsyncStorage.setItem(`consecutiveDays_${currentUserId}`, consecutiveDays.toString())
        console.log(`[HomeScreen] Meta atingida para ${currentUserId}! Dias consecutivos: ${consecutiveDays}`)
      }
      ...
      await AsyncStorage.setItem(`waterConsumption_${currentUserId}`, JSON.stringify(waterConsumptionData))

      // Atualizar conquistas e desafios diretamente
      await updateAchievementsAndChallenges(todayTotal, recommendedWaterIntakeLiters, consecutiveDays)
    } catch (error) {
      console.error(`[HomeScreen] Erro ao atualizar conquistas:`, error)
    }
  }
  ...
  const updateAchievementsAndChallenges = async (
    ...
    try {
      // Carregar conquistas e desafios atuais
      const savedAchievementsStr = await AsyncStorage.getItem(`achievements_${currentUserId}`)
      const savedChallengesStr = await AsyncStorage.getItem(`challenges_${currentUserId}`)

      if (!savedAchievementsStr || !savedChallengesStr) {
        console.error(`[HomeScreen] Conquistas ou desafios não encontrados no AsyncStorage para ${currentUserId}`)
        return
      }
      ...
      for (let i = 0; i < challenges.length; i++) {
        ...
        if (challenge.id === "challenge_2" && !challenge.completed && todayConsumption >= dailyGoal) {
          challenge.completed = true
          challengesUpdated = true
          console.log(`[HomeScreen] Desafio de meta diária completado para ${currentUserId}!`)
          ...
          if (relatedAchievement && !relatedAchievement.completed) {
            relatedAchievement.completed = true
            achievementsUpdated = true
            console.log(`[HomeScreen] Conquista de meta diária desbloqueada para ${currentUserId}!`)
          }
        }
        ...
        if (challenge.id === "challenge_3" && !challenge.completed && todayConsumption >= 2) {
          challenge.completed = true
          challengesUpdated = true
          console.log(`[HomeScreen] Desafio de 2 litros completado para ${currentUserId}!`)
          ...
          if (relatedAchievement && !relatedAchievement.completed) {
            relatedAchievement.completed = true
            achievementsUpdated = true
            console.log(`[HomeScreen] Conquista de 2 litros desbloqueada para ${currentUserId}!`)
          }
        }
        ...
        if (challenge.id === "challenge_1" && !challenge.completed && consecutiveDays >= 7) {
          challenge.completed = true
          challengesUpdated = true
          console.log(`[HomeScreen] Desafio de 7 dias consecutivos completado para ${currentUserId}!`)
          ...
          if (relatedAchievement && !relatedAchievement.completed) {
            relatedAchievement.completed = true
            achievementsUpdated = true
            console.log(`[HomeScreen] Conquista de 7 dias consecutivos desbloqueada para ${currentUserId}!`)
          }
        }
      }
      ...
      // Salvar conquistas e desafios atualizados
      if (achievementsUpdated) {
        await AsyncStorage.setItem(`achievements_${currentUserId}`, JSON.stringify(achievements))
        console.log(`[HomeScreen] Conquistas atualizadas e salvas com sucesso para ${currentUserId}`)
      }

      if (challengesUpdated) {
        await AsyncStorage.setItem(`challenges_${currentUserId}`, JSON.stringify(challenges))
        console.log(`[HomeScreen] Desafios atualizados e salvos com sucesso para ${currentUserId}`)
      }
    } catch (error) {
      console.error(`[HomeScreen] Erro ao atualizar conquistas e desafios:`, error)
    }
  }
  ...
  const resetWaterConsumed = async (isDailyReset = false) => {
    ...
    // Salvar o timestamp no AsyncStorage
    try {
      await AsyncStorage.setItem(`lastResetTimestamp_${currentUserId}`, resetTimestamp.toString())
      console.log(`[HomeScreen] Timestamp de reinício salvo para ${currentUserId}:`, resetTimestamp)
      ...
      if (isDailyReset) {
        // Resetar dias consecutivos apenas se for um reset diário
        await AsyncStorage.setItem(`consecutiveDays_${currentUserId}`, "0")

        // Resetar conquistas e desafios
        await resetAchievements()
      }
    } catch (error) {
      console.error(`[HomeScreen] Erro ao salvar timestamp de reinício:`, error)
    }

    console.log(`[HomeScreen] Contador de água consumida zerado para ${currentUserId}`)
  }
  ...
  const resetAchievements = async () => {
    try {
      ...
      // Salvar conquistas e desafios resetados
      await AsyncStorage.setItem(`achievements_${currentUserId}`, JSON.stringify(resetAchievements))
      await AsyncStorage.setItem(`challenges_${currentUserId}`, JSON.stringify(resetChallenges))

      console.log(`[HomeScreen] Conquistas e desafios resetados com sucesso para ${currentUserId}`)
      ...
      console.error(`[HomeScreen] Erro ao resetar conquistas:`, error)
    }
  }
  ...
  const handleWaterConsumption = async () => {
    ...
    if (amountInMl > 0) {
      ...
      if (modalMode === "add") {
        ...
        console.log(`[HomeScreen] Novo item criado para ${currentUserId}:`, JSON.stringify(newItem))
        // Adicionar o novo item ao início do histórico
        updatedHistory = [newItem, ...historyItems]
      } else {
        // Modo de edição - atualizamos o item existente
        ...
        console.log(`[HomeScreen] Item editado para ${currentUserId}:`, JSON.stringify(updatedHistory[editIndex]))
      }
      ...
      // Salvar o histórico atualizado no AsyncStorage
      await saveHistoryItems(updatedHistory)
      console.log(`[HomeScreen] Histórico atualizado e salvo com sucesso para ${currentUserId}`)
      ...
      // Mostrar no console o novo total
      console.log(`[HomeScreen] ${newTotal} Litros consumidos hoje por ${currentUserId}`)
